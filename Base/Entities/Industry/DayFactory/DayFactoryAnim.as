...
void onInit(CSprite@ this)
{
	...
	LayerSetup[] layers =
	{
		...
		makeLayerSetup("gear 4",		texname, Vec2f(8, 8),	team_cog,		Vec2f(-6, 7),	team, 0,	true, rotates | forwards),
...
void onTick(CSprite@ this)
{
	...
	if (blob is null || !blob.get("layer setups", @layers))
	{
		return;
	}

	const bool producing = !blob.hasTag("production paused");
	...
	for (uint step = 0; step < layers.length; ++step)
	{
		...
		if (!hasTech)
		{
			if (step == 5 && blob.hasTag("icon layer")) //HACK
			{
...
void AddSignLayerFrom(CSprite@ this)
{
	...
	s8 x = blob.get_s8("sign x");
	s8 rot = blob.get_s8("sign rotation");
	...
	CSpriteLayer@ icon = this.addSpriteLayer("icon", blob.get_string("icon file"), 16, 16, team, skin);
	if (icon !is null)
	{
		Animation@ anim = icon.addAnimation("default", 0, false);
		anim.AddFrame(blob.get_u8("icon frame"));
		icon.SetOffset(Vec2f(blob.get_string("tech name") == "mat_arrows" && rot <= 0 ? x + 1 : x, -12 + blob.get_s8("sign offset")));
		icon.SetRelativeZ(1015);
		icon.RotateBy(rot, Vec2f());
		f32 scale = blob.get_f32("sign scale");
		icon.ScaleBy(scale, scale);
	}
}
...
void onRender(CSprite@ this)
{
	   ...
	   )
	{
		if (blob.hasTag("production paused") && blob.get_string("tech name").size() > 0)
		{
			...
			GUI::SetFont("menu");
			GUI::DrawText(getTranslatedString("Requires <magic material name> to operate."), upperleft, lowerright, SColor(255, 240, 10, 10), false, false, true);
		}
	}  // E
}
*/