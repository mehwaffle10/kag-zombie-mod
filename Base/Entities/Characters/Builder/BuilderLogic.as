...
#include "ZombieBlocksCommon.as";  // Waffle: Support modded blocks
...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
	...
	else
	{
		...
		if (blob !is null)
		{
            ...
            // Waffle: Prevent breaking static blobs in no build zone
            if (isStationary(blob) && map !is null && map.getSectorAtPosition(tilepos, "no build") !is null)
            {
                return true;
            }
...
void Pickaxe(CBlob@ this)
{
	...
	if (!adjusttime)
	{
		if (!justCheck)
		{
			...
			if (hitdata.blobID == 0)
			{
				...
				if (!isEmpty(t) && !isDirtBackwall(t))  // Waffle: Support modded blocks
				{
                    ...
                    if (sector !is null)
                    {
                        ...
                        hitdata.upper_left = sector.upperleft;
                        hitdata.lower_right = sector.lowerright;
                        hitdata.ticks = 2 * getTicksASecond();
                    }
                    else
                    {
			...
			else
			{
				...
				if (b !is null)
				{
                    Vec2f hit_pos = (b.getPosition() + this.getPosition()) * 0.5f;
                    CMap::Sector@[] sectors;
                    CMap::Sector@ sector = null;
                    map.getSectorsAtPosition(hit_pos, sectors);
                    for (u8 i = 0; i < sectors.length; i++)
                    {
                        if (sectors[i] !is null && sectors[i].ownerID != b.getNetworkID())
                        {
                            @sector = @sectors[i];
                            break;
                        }
                    }
                    if (isStationary(b) && sector !is null)
                    {
                        this.getSprite().PlaySound("NoAmmo.ogg", 0.5);
                        hitdata.upper_left = sector.upperleft;
                        hitdata.lower_right = sector.lowerright;
                    ...
                    {
					    SendHitCommand(this, b, hit_pos, attackVel, hit_damage);
                    }
				}
			}
		}
	...
	if ((tilepos - aimPos).Length() < bestDistance - 4.0f && map.getBlobAtPosition(tilepos) is null)
	{
		...
		if (!isDirtBackwall(tile.type))
		{
			...
			else if (isGrass(tile.type))  // Waffle: Support modded blocks
			{
...
bool isStationary(CBlob@ blob)
{
    return blob is null || blob.hasTag("aligned to tiles") || blob.hasTag("door") || blob.isLadder() || blob.isPlatform();
}
